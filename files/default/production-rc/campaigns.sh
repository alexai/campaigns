...
DYNATRACE="";
ping -c 1 -i 1 -t 1 dfdtcollector1.demandforce.com;
DYNALIVE=$?;

if [ ${DYNALIVE} -eq 0 ]; then
        DYNATRACE="-agentpath:/home/dynatrace/dynatrace-5.0.0/agent/lib64/libdtagent.so=name=Campaigns_Demandforce-$HOSTNAME,server=dfdtcollector1.demandforce.com:9998";
fi
...
nohup $JAVA_HOME/bin/java ${DYNATRACE} ${JMX} -cp classes:$CLASSPATH/'*' $PID_PROPERTY $CONFIG_OVERRIDE $LOG4J_OVERRIDE $JNDI_OVERRIDE com.demandforce.campaign.Manager <&- &
